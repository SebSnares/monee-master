...
if [ ${FLAGS_halfselection} -eq ${FLAGS_TRUE} ]; then
  USEHALFSELECTIONREP=s/--USE_HALFSELECTION/true/g
else
  USEHALFSELECTIONREP=s/--USE_HALFSELECTION/false/g
fi
...
if [ -n "${FLAGS_logdir}" ]
then
    echo "Copying results to ${FLAGS_logdir}" >> ${HOME}/monee/monee-master/moneesh-logs.txt
