...
#include <algorithm>
#include <set>
#include <math.h> //library for Eulers number
...
std::vector<Genome>::const_iterator SimpleShellsControlArchitecture::selectWeighted(std::vector<Genome> & genomes) {
	...
	// else
	int halfselect = 0;
	std::cout << "_useHalfSelection:"  << _useHalfSelection <<'\n';
	if (_useHalfSelection){ //If half selection is turned on (=true) then 50/50 chance of using random selection (=1) else dont affect random selection (=0)
		...
		std::cout << "halfselect:"  << halfselect <<'\n';
	}
...
void SimpleShellsControlArchitecture::assignFitness(std::vector<Genome> & genomes) {
	...
	std::cout << "desired ratio:" << desiredRatio <<'\n'; //print desiredration (should be 0.5)
	for (int i = 0; i < gPuckColors; i++) {
		if (i != _wm->_energyPuckId || !_wm->_excludeEnergyPucks) {
